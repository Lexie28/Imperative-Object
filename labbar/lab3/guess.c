...
int main() {
    ...
    char *name = ask_question_string("Skriv in dit namn: \n");       //frågar namn, tar in ditt namn när du skriver det, returnerar ditt namn (inte printar!! bara returnar)//

    printf("Du %s, ", name);

    current_guess = ask_question_int("jag tänker på ett tal... kan du gissa vilket?\n");  // current guess = funktionen tar in en fråga, läser och returnerar ett tal från tangentbord som vi fångar och sparar i current guess//
    
    while(num_of_tries < 15) {
